    ...
    return {
        ...
        serveDocument: async (req, res) => {
            try {
                ...
                console.log(`📥 Agent Review: Serving document ${documentId} for client ${clientId}`);
                ...
                if (!document) {
                    console.log(`❌ Document not found in client records: ${documentId}`);
                ...
                // 1. Support for test scenarios (Mock PDF)
...
<< /Length 120 >>
stream
BT /F1 12 Tf 50 700 Td (Mock Test Document: ${document.name}) Tj 0 -20 Td (This is a test document for Agent Review Dashboard) Tj ET
endstream
endobj
xref
0 5
...
%%EOF`;
                ...
                res.setHeader('Content-Disposition', `inline; filename="${document.name || filename}"`);

                try {
                    const fileStream = getGCSFileStream(decodedFilename);
                    fileStream.on('error', (err) => {
                        console.error(`❌ GCS stream error for ${decodedFilename}:`, err.message);
                        if (!res.headersSent) {
