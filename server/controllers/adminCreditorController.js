...
/**
 * Admin Creditor Database Controller
 * Handles CRUD, Import, and Export operations for the global creditor database
 */


/**
 * Admin Creditor Database Controller Factory
 * Handles CRUD, Import, and Export operations for the global creditor database
 * Dependencies: Client, documentProcessor (for creditor document processing)
 */
const createAdminCreditorController = ({ Client, documentProcessor, safeClientUpdate }) => {
    return {
        // List creditors with search and pagination
        listCreditors: async (req, res) => {
            try {
        ...
        // Import creditors from Excel
        importCreditors: async (req, res) => {
            try {
                if (!req.file) {
        ...
        /**
         * Process documents to extract creditors
         * POST /api/clients/:clientId/process-documents-to-creditors
         * (Formerly in server.js test area)
         */
        processDocumentsToCreditors: async (req, res) => {
            try {
                ...
                console.log(`Processing documents for client ${client.aktenzeichen}...`);

                if (!documentProcessor) {
                    throw new Error('documentProcessor dependency is missing');
                }

                // Process documents (placeholder logic calling service)
                const result = await documentProcessor.processClientDocuments(client.aktenzeichen, documentId);

                // Update client state if needed
                // If service returns extracted creditors, we might save them?
                // The service likely updates the client or returns data.
                // Assuming service handles updates or we restart client/reload.
                // Server.js code just called: const result = await documentProcessor.processClientDocuments(client.aktenzeichen);
                // And returned result.
...
module.exports = createAdminCreditorController;
