...
class CreditorDocumentPackageGenerator {
    ...
    async generateAllWordDocuments(clientData, settlementData, clientReference) {
        ...
        try {
            ...
            let schuldenplan;
            if (isNullplan) {
                console.log('  📝 Client has no pfändbares Einkommen - generating Nullplan');
                // For Nullplan, generate all documents via generateNullplanDocuments
                const nullplanResult = await this.documentGenerator.generateNullplanDocuments(clientReference);
                if (nullplanResult.success) {
                    schuldenplan = nullplanResult.nullplan;
                    // Also store the other documents for later use
                    documents.forderungsuebersicht = nullplanResult.forderungsuebersicht;
                    documents.ratenplan = nullplanResult.ratenplan_nullplan;
                    console.log('  ✅ Nullplan documents generated (all 3)');
                } else {
                    throw new Error(nullplanResult.error || 'Nullplan generation failed');
                }
            } else {
                schuldenplan = await this.documentGenerator.generateSettlementPlanDocument(
                    clientReference,
                    settlementData
                );
            }
        ...
        // 2. Generate Forderungsübersicht (skip if already generated by Nullplan)
        if (!documents.forderungsuebersicht) {
            try {
            ...
            console.log('  ✅ Forderungsübersicht already generated (from Nullplan)');
        }

        // 3. Generate Ratenplan pfändbares Einkommen (skip if already generated by Nullplan)
        if (!documents.ratenplan) {
            try {
            ...
            console.log('  ✅ Ratenplan already generated (from Nullplan)');
        }
